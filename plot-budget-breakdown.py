...
def ap_to_sector(ap: str, org: str) -> str:
    ...
    if (
        ap == "SERVICIOS PÚBLICOS GENERALES"
        and org == "Intereses y otros Gastos de la Deuda"
    ):
        return DEUDA
...
def load_1961_2010() -> dict[int, dict[str, float]]:
    text = Path("./data-budget/opp_1961_2010.csv").read_bytes().decode("utf-8-sig")
    reader = csv.DictReader(io.StringIO(text), delimiter=";")

    totals: defaultdict[int, defaultdict[str, float]] = defaultdict(
...
def load_2011_2025() -> dict[int, dict[str, float]]:
    text = Path("./data-budget/opp_2011_2025.csv").read_bytes().decode("utf-8")
...
def load_wb_gdp_const() -> dict[int, float]:
    result = {}
    with open("./data-budget/wb_gdp_const_usd.csv") as f:
...
def load_wb_gdp_nominal() -> dict[int, float]:
    result = {}
    with open("./data-budget/wb_gdp_nominal_pesos.csv") as f:
        for row in csv.DictReader(f):
...
for yr in years_sorted:
    ...
    budget_real_usd_billions_total = (
        budget_pct_gdp[yr] / 100 * gdp_real_usd_billions[yr]
    )
...
def stacked_area(
    ...
    for sector in SECTORS:
        ys = np.array([year_val_dict[y].get(sector, 0) for y in yrs])
        ax.fill_between(
            xs,
            bottom,
            bottom + ys,
...
def plot_stacked_pct() -> plt.Figure:
    ...
    fig.text(0.5, -0.03, FOOTNOTE, ha="center", fontsize=7.5, alpha=0.7)
    return fig


def plot_real_usd() -> plt.Figure:
    fig, ax = plt.subplots(figsize=(36 * cm, 16 * cm), layout="constrained")
    stacked_area(
        ax,
        budget_real_usd_billions,
...
def plot_lines() -> plt.Figure:
    fig, ax = plt.subplots(figsize=(36 * cm, 16 * cm), layout="constrained")
    yrs = sorted(pct_budget)
    xs = np.array(yrs)
    for sector in SECTORS:
        ys = np.array([pct_budget[y].get(sector, 0) for y in yrs])
        ax.plot(
            xs, ys, color=SECTOR_COLORS[sector], linewidth=1.8, label=sector, zorder=2
        )
...
# --- select chart ---
fig = plot_stacked_pct()
# fig = plot_real_usd()
# fig = plot_lines()

file_stem = Path(__file__).stem
for fmt in ("svg", "png"):
    fig.savefig(f"{file_stem}.{fmt}", dpi=200, bbox_inches="tight", pad_inches=0.5)
os.execlp("open", "open", f"./{file_stem}.png")
